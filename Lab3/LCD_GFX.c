...
*****************************************************************************/
char String[25];
void LCD_drawCircle(uint8_t x0, uint8_t y0, uint8_t radius, uint16_t fg_color, uint16_t bg_color)
{
    ...
    for(int16_t i=x1; i <= x2; i++) {
        for(int16_t j=y1; j <= y2; j++) {
            ...
            {
                SPI_ControllerTx_16bit_stream(fg_color);
            }
            else
            {
                SPI_ControllerTx_16bit_stream(bg_color);
            }
        }
    }
