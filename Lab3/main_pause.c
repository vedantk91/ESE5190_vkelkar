...
int led_on_count=0;
char cpu[10]="Player 2";
char player[10]="Player 1";
...
int main(void)
{
    ...
    while (1) 
    {
		int buttonPress = (PIND & (1<<2));
		int lastButtonPress=0;
		
		if(buttonPress!=0 && lastButtonPress==0){
			pause ^= 1;
		}
		...
		if(button1state) {
			del_x_paddle = speed;
			sprintf(debug_string, "UP");
			UART_putstring(debug_string);
		}
		else if (button2state) {
			del_x_paddle = -speed;
			sprintf(debug_string, "DOWN");
			UART_putstring(debug_string);
		}
		else {
			del_x_paddle = 0;
			sprintf(debug_string, "ZERO");
			UART_putstring(debug_string);
		}
        ...
        if(user_score == 6){
            char print_str[20] = "PLAYER 1 WINS!";
            LCD_setScreen(BLACK);
            ...
            char print_str[20] = "PLAYER 2 WINS!";
            LCD_setScreen(BLACK);
            LCD_drawString(50,60,print_str,WHITE,BLACK);
            _delay_ms(3000);
            LCD_setScreen(BLACK);
